...
m1, m2, m3, m4, m5, m6, m7, m8, m9, m10 = [], [], [], [], [], [], [], [], [], []  # Пустные списки
...
for i in x:
    if min(x) <= i <= min(x) + h:
        ...
        m9.insert(0, i)
    elif min(x) + 9 * h <= i <= min(x) + 10 * h:
        m10.insert(0, i)

m1, m2, m3, m4, m5, m6, m7, m8, m9, m10 = len(m1), len(m2), len(m3), len(m4), len(m5), len(m6), len(m7), len(m8), len(
    m9), len(m10)

# Частота попадания
p1, p2, p3, p4, p5, p6, p7, p8, p9, p10 = m1 / n, m2 / n, m3 / n, m4 / n, m5 / n, m6 / n, m7 / n, m8 / n, m9 / n, m10 / n

# Плотность частоты
f1, f2, f3, f4, f5, f6, f7, f8, f9, f10 = round(p1 / h, 3), round(p2 / h, 3), round(p3 / h, 3), round(p4 / h, 3), round(
    p5 / h, 3), round(p6 / h, 3), round(p7 / h, 3), round(p8 / h, 3), round(p9 / h, 3), round(p10 / h, 3)

# Середина интервалов
x1, x2, x3, x4, x5, x6, x7, x8, x9, x10 = round((2 * min(x) + h) / 2, 3), round((2 * min(x) + 3 * h) / 2, 3), round(
    (2 * min(x) + 5 * h) / 2, 3), round(
    (2 * min(x) + 7 * h) / 2, 3), round((2 * min(x) + 9 * h) / 2, 3), round((2 * min(x) + 11 * h) / 2, 3), round(
    (2 * min(x) + 13 * h) / 2, 3), round((2 * min(x) + 15 * h) / 2, 3), round((2 * min(x) + 17 * h) / 2, 3), round(
    (2 * min(x) + 19 * h) / 2, 3)
